...
## フェーズ別タスク
### Phase 0 — 調査
- [ ] `NOTE_SELECTORS.LIST_CONTAINER` の `display` が `flex/grid` か実測し、`order` 使用可否を決める。
- [ ] スクロール時の DOM 差し替えを観測し、仮想化の有無と頻度を把握する。
- [ ] `aria-labelledby` 由来の noteId 抽出が安定するか確認する。
- [ ] ヘッダー挿入方式（DOM 挿入 vs `::before`）の A11y 影響を比較する。
- [ ] DevTools で `LIST_CONTAINER` の computed style を記録する（`display`/`gap`/`flex-direction`）。
- [ ] 50〜100 ノートでスクロール時の DOM 数変化をログ化する。
- [ ] `MutationObserver` でノートカードの追加/削除の発生頻度をメモする。
- [ ] ノートカードの親タグ（`ul`/`div` 等）と子タグの種類を控える。

### Phase 1 — 設定/状態
- [ ] `viewMode` の初期値と復元タイミングを定義する。
- [ ] `storageManager.settings.viewMode` の保存/復元を実装する。
- [ ] `filterManager` の状態に `viewMode` を統合する。
- [ ] 元の DOM 順序インデックスの保存/再初期化ルールを実装する。
- [ ] `filterManager` に `applyViewMode(mode)` を追加する。
- [ ] `filterManager.selectFolder()` に `viewMode` 適用の呼び出しを追加する。
- [ ] `domRecoveryManager` から `applyViewMode` を再呼び出しできる導線を作る。
- [ ] `settings` からの復元は `load` 完了後に適用する方針を明文化する。
